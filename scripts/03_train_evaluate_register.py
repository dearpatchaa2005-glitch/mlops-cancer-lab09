...
def train_evaluate_register(preprocessing_run_id, C=1.0):
    ...
    both the accuracy and ROC-AUC performance thresholds.
    """
    ACCURACY_THRESHOLD = 0.95
    ROC_AUC_THRESHOLD = 0.98
    MODEL_NAME = "cancer-classifier-prod"

    mlflow.set_experiment("Cancer Quality - Model Training")
    ...
    with mlflow.start_run(run_name=f"logistic_regression_C_{C}"):
        ...
        # 3. ประเมินผลโมเดล — accuracy และ roc_auc (เพราะเป็น binary classification)
        ...
        # 5. ตรวจสอบและลงทะเบียนโมเดล — ต้องผ่านทั้ง 2 เกณฑ์
        if acc >= ACCURACY_THRESHOLD and roc_auc >= ROC_AUC_THRESHOLD:
            print(
                f"Model accuracy ({acc:.4f}) and ROC-AUC ({roc_auc:.4f}) "
                "meet the thresholds. Registering model..."
            )
            ...
            print(
                f"Model accuracy ({acc:.4f}) or ROC-AUC ({roc_auc:.4f}) "
                "is below the threshold. Not registering."
            )
